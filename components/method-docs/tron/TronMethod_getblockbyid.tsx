...
export function TronMethod_getblockbyid(props: GenericMethodPropsReplacing) {
  return (
    <TronMethod
      method="getblockbyid"
      network="tron"
      cu={20}
      description={"Returns block details for a specific block hash (block ID)."}
      useCases={USE_CASES}
      ...
      responseParamsDescription={
        "Returns block details for a specific block hash (block ID)."
      }
      {...props}
    />
  );
...
const CODE_SNIPPETS: Array<CodeSnippetObject> = [
  {
    ...
    code: () => `curl --request POST \\
     --url ${DRPC_ENDPOINT_URL_TRON_PAID}/wallet/getblockbyid \\
  ...
  {
    language: "js",
    code: () => `const url = '${DRPC_ENDPOINT_URL_TRON_PAID}/wallet/getblockbyid';
...
const url = '${DRPC_ENDPOINT_URL_TRON_PAID}/wallet/getblockbyid';
...
const REQUEST_PARAMS: RequestParamProp = [
  {
    paramName: "value",
    type: "string",
    paramDescription: "[Required] The block hash (block ID) to look up",
  },
];
...
const USE_CASES = [
  "Look up a block a user references by its hash rather than its number",
];

const CONSTRAINTS = [
  "Returns an empty object if no block with that hash exists",
